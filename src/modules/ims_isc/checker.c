...
static int isc_check_headers(ims_spt *spt, struct hdr_field *headers)
{
	...
	if(spt->sip_header.header.len >= sizeof(buf)) {
		LM_ERR("Header name \"%.*s\" is to long to be processed (max %d "
			   "bytes)\n",
	...
	if(spt->sip_header.content.len >= sizeof(buf)) {
		LM_ERR("Header content \"%.*s\" is to long to be processed (max %d "
			   "bytes)\n",
	...
	if(regcomp(&(content_comp), buf, REG_ICASE | REG_EXTENDED) != 0) {
		LM_ERR("Error compiling the following regexp for header content: "
			   "%.*s\n",
				spt->sip_header.content.len, spt->sip_header.content.s);
		regfree(&(header_comp));
		return FALSE;
	...
	while(i != NULL) {
		...
		if((spt->sip_header.type > 0 && spt->sip_header.type == i->type)
				|| //matches known type
				(regexec(&(header_comp), i->name.s, 0, NULL, 0)
						== 0) //or matches the name
		) {
			...
			if(regexec(&(content_comp), i->body.s, 0, NULL, 0)
					== 0) //regex match
			{
				regfree(&(header_comp));
				regfree(&(content_comp));
...
static int isc_check_session_desc(ims_spt *spt, struct sip_msg *msg)
{
	...
	body[len] = 0;
	x = pkg_malloc(
			spt->session_desc.line.len + 2 + spt->session_desc.content.len);
...
static int isc_check_ruri(ims_spt *spt, struct sip_msg *msg)
{
	...
	if(msg->first_line.u.request.uri.len >= sizeof(buf2)) {
		LM_ERR("RURI \"%.*s\" is too long to be processed (max %d bytes)\n",
				...
				msg->first_line.u.request.uri.s, (int)(sizeof(buf2) - 1));
		return FALSE;
	}
	...
	memcpy(buf2, msg->first_line.u.request.uri.s,
			msg->first_line.u.request.uri.len);
	...
	for(i = 0; i < t->spt_cnt; i++) {
		if(group != t->spt[i].group) { //jump to other group
			...
			partial = t->condition_type_cnf == IFC_CNF
							  ? partial
										|| isc_check_spt(t->spt + i, msg,
												direction, registration_type)
							  : partial
										&& isc_check_spt(t->spt + i, msg,
												direction, registration_type);
		}
		...
		struct sip_msg *msg, int registered, udomain_t *d)
{
	...
	if((ret = isc_ulb.get_impurecord(d, &uri, &p)) != 0) {
		...
		LM_ERR("Failure getting IMPU record for [%.*s] - ISC checker find "
			   "METHOD: [%.*s]\n",
