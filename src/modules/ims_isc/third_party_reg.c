...
static inline unsigned int calc_associateduri_buf_len(ims_subscription *s)
{
	...
	if(len)
		len += PASSOCIATEDURI_LEN + 2 + CRLF_LEN;
...
int build_p_associated_uri(ims_subscription *s)
{
	...
	LM_DBG("Created P-Associated-URI HF %.*s\n", p_associated_uri.data_len,
			p_associated_uri.buf);
	...
	if(req_uri.s) {
		...
		if(m->service_info.s && m->service_info.len) {
			...
			r.body.content.s = build_res_buf_from_sip_req(200, &reg_resp_200OK,
					0, msg, (unsigned int *)&r.body.content.len, &dummy_bm);
			if(!r.body.content.s) {
				LM_DBG("response building failed for body of third party "
					   "register request");
