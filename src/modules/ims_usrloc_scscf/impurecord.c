...
static ucontact_t *contacts_to_expire
		[MAX_CONTACTS_PER_IMPU]; //this is done to prevent fragmentation of memory...
 ...
 */
int new_impurecord(str *_dom, str *public_identity, str *private_identity,
		int reg_state, int barring, ims_subscription **s, str *ccf1, str *ccf2,
		str *ecf1, str *ecf2, impurecord_t **_r)
{
	*_r = (impurecord_t *)shm_malloc(sizeof(impurecord_t));
	...
	memcpy((*_r)->private_identity.s, private_identity->s,
	...
	if(barring
			>= 0) { //just in case we call this with no barring -1 will ignore
		(*_r)->barring = barring;
	}
	(*_r)->send_sar_on_delete = 1; /*defaults to 1 */
	if(ccf1 && ccf1->len > 0)
		STR_SHM_DUP((*_r)->ccf1, *ccf1, "CCF1");
	if(ccf2 && ccf2->len > 0)
		STR_SHM_DUP((*_r)->ccf2, *ccf2, "CCF2");
	if(ecf1 && ecf1->len > 0)
		STR_SHM_DUP((*_r)->ecf1, *ecf1, "ECF1");
	if(ecf2 && ecf2->len > 0)
		STR_SHM_DUP((*_r)->ecf2, *ecf2, "ECF2");
...
void print_impurecord(FILE *_f, impurecord_t *_r)
{
	...
	if(_r->s) {
		fprintf(_f,
				"\tIMS service profiles count (%d):   '%p' (refcount: %d)\n",
	...
	while(subscriber) {
		...
		fprintf(_f, "\t\twatcher uri: <%.*s> and presentity uri: <%.*s>\n",
				subscriber->watcher_uri.len, subscriber->watcher_uri.s,
				subscriber->presentity_uri.len, subscriber->presentity_uri.s);
...
void mem_delete_ucontact(ucontact_t *_c)
{
	...
	for(dialog_data = _c->first_dialog_data; dialog_data;) {
		...
		dlgb.lookup_terminate_dlg(
				dialog_data->h_entry, dialog_data->h_id, NULL);
...
static str fail_isolation_level =
		str_init("SET TRANSACTION ISOLATION LEVEL READ COMMITTED");
...
static inline void process_impurecord(impurecord_t *_r)
{
	...
	LM_DBG("\tPublic Identity %.*s, Barred: [%d], State: [%s], contacts [%d], "
		   "3gppcontacts [%d]\n",
			_r->public_identity.len, _r->public_identity.s, _r->barring,
	...
	while(impu_contact) {
		...
		if(!VALID_CONTACT(ptr, act_time)) {
			if(ptr->state == CONTACT_DELETED) {
				...
				LM_DBG("Contact: <%.*s> is in state "
					   "CONTACT_EXPIRE_PENDING_NOTIFY....running callback\n",
						ptr->c.len, ptr->c.s);
				if(exists_ulcb_type(_r->cbs, UL_IMPU_EXPIRE_CONTACT)) {
					LM_DBG("Running callback UL_IMPU_EXPIRE_CONTACT for "
						   "contact [%.*s] and impu [%.*s]\n",
							ptr->c.len, ptr->c.s, _r->public_identity.len,
				...
				LM_DBG("Contact: <%.*s> is in state valid but it has "
					   "expired.... ignoring as the contact check will set the "
	...
	if(num_contacts_to_expire > 0) {
		...
		for(n = 0; n < num_contacts_to_expire; n++) {
			...
			LM_DBG("\t\texpiring contact %i: [%.*s] in slot [%d]\n", n,
					contacts_to_expire[n]->c.len, contacts_to_expire[n]->c.s,
					contacts_to_expire[n]->sl);
			sl = ptr->sl;
			lock_contact_slot_i(sl);
			if(!dbwork) {
				start_dbtransaction();
			...
			unlink_contact_from_impu(
					_r, ptr, 1, 0 /*implicit dereg of contact from IMPU*/);
			unlock_contact_slot_i(sl);
		}
	}
	...
	while(s) {
		...
		if(!valid_subscriber(s, act_time)) {
			LM_DBG("DBG:registrar_timer: Subscriber with watcher_contact "
				   "<%.*s> and presentity uri <%.*s> expired and removed.\n",
					s->watcher_contact.len, s->watcher_contact.s,
			...
			LM_DBG("DBG:registrar_timer: Subscriber with watcher_contact "
				   "<%.*s> and presentity uri <%.*s> is valid and expires in "
				   "%d seconds.\n",
					s->watcher_contact.len, s->watcher_contact.s,
					s->presentity_uri.len, s->presentity_uri.s,
			...
			LM_DBG("Searching sub dialog hash info with call_id: <%.*s> and "
				   "ttag <%.*s> ftag <%.*s> and hash code <%i>\n",
					s->call_id.len, s->call_id.s, s->to_tag.len, s->to_tag.s,
					s->from_tag.len, s->from_tag.s, sl);
			/* search the record in hash table */
			lock_get(&sub_dialog_table[sl].lock);
			sub_dialog = pres_search_shtable(
					sub_dialog_table, s->call_id, s->to_tag, s->from_tag, sl);
			if(sub_dialog == NULL) {
				LM_ERR("DBG:registrar_timer: Subscription has no dialog record "
					   "in hash table\n");
			} else {
	...
	if(mustdeleteimpu) {
		...
		if(!hascontacts) {
			...
			if(_r->reg_state != IMPU_UNREGISTERED
					&& exists_ulcb_type(_r->cbs, UL_IMPU_UNREG_NC)) {
 ...
 */
int insert_scontact(
		impurecord_t *_r, str *_contact, ucontact_info_t *_ci, ucontact_t **_c)
{
	...
	if(maxcontact > 0 && maxcontact_behaviour > 0) {
		...
		if(numcontacts >= maxcontact) {
			switch(maxcontact_behaviour) {
				case 1: //reject
					LM_ERR("too many contacts already registered for IMPU "
						   "<%.*s>\n",
							_r->public_identity.len, _r->public_identity.s);
					return -1;
				case 2: //overwrite oldest
					LM_DBG("Too many contacts already registered, overwriting "
						   "oldest for IMPU <%.*s>\n",
							_r->public_identity.len, _r->public_identity.s);
	...
	if(maxcontact_3gpp > 0 && maxcontact_behaviour > 0) {
		...
		if(num_3gpp_contacts >= maxcontact_3gpp) {
			switch(maxcontact_behaviour) {
				case 1: //reject
					LM_ERR("too many 3GPP contacts already registered for IMPU "
						   "<%.*s>\n",
							_r->public_identity.len, _r->public_identity.s);
					return -1;
				case 2: //overwrite oldest
					LM_DBG("Too many 3GPP contacts already registered, "
						   "overwriting oldest for IMPU <%.*s>\n",
							_r->public_identity.len, _r->public_identity.s);
...
int delete_scontact(struct ucontact *_c)
{
	...
	if(db_mode == WRITE_THROUGH && db_delete_ucontact(_c) != 0) {
		LM_ERR("error removing contact from DB [%.*s]... will still remove "
			   "from memory\n",
				_c->c.len, _c->c.s);
	}
...
static inline struct ucontact *contact_match(unsigned int slot, str *_c)
{
	...
	while(ptr) {
		if((ptr->state != CONTACT_DELAYED_DELETE) && (_c->len == ptr->c.len)
				&& !memcmp(_c->s, ptr->c.s, _c->len)) { //check validity
			return ptr;
		}
		ptr = ptr->next;
	}
...
static inline struct ucontact *contact_port_ip_match(unsigned int slot, str *_c)
{
	...
	while(ptr) {
		...
		if((ptr->state != CONTACT_DELAYED_DELETE)
				...
				&& !memcmp(string_ip_port.s, contact_ip_port.s,
						string_ip_port.len)) {
			return ptr;
		}
...
static inline struct ucontact *contact_callid_match(
		unsigned int slot, str *_c, str *_callid)
{
	ucontact_t *ptr = contact_list->slot[slot].first;

	while(ptr) {
		if((ptr->state != CONTACT_DELAYED_DELETE) && (_c->len == ptr->c.len)
				&& (_callid->len == ptr->callid.len)
		...
		str *_c, str *_callid, str *_path, int _cseq, struct ucontact **_co)
{
	...
	*_co = 0;
	...
	if(ptr) {
		...
		if(!with_callid
				|| (_callid && ptr->callid.len == _callid->len
						&& memcmp(_callid->s, ptr->callid.s, _callid->len)
								   == 0)) {
			if(_cseq < ptr->cseq) {
		...
		*_co = ptr;
		unlock_contact_slot_i(
				sl); /*TODO: we probably need to ref count here..... */
		return 0;
	}
	unlock_contact_slot_i(sl);
	...
	for(i = 0; i < orig->service_profiles_cnt; i++) {
		for(j = 0; j < orig->service_profiles[i].public_identities_cnt; j++) {
			for(k = 0; k < new->service_profiles_cnt; k++) {
				for(l = 0; l < new->service_profiles[k].public_identities_cnt;
						l++) {
 ...
 */
int update_impurecord(struct udomain *_d, str *public_identity,
		...
		str *ecf1, str *ecf2, struct impurecord **_r)
{
	...
	if(!impu_rec
			&& (!public_identity || !public_identity->len
					|| !public_identity->s)) {
	...
	if(s && *s) {
		subs_ptr = (*s);
		res = get_subscription(&(*s)->private_identity, &subscription,
				leave_slot_locked); //leave slot locked in case we need to add.... don't want racing adds
		if(res != 0) {
			LM_DBG("No subscription yet for [%.*s]... adding\n",
					(*s)->private_identity.len, (*s)->private_identity.s);
			ref_subscription_unsafe(
	...
	if(impu_rec) {
		...
		if(res != 0) {
			if(reg_state != IMPU_NOT_REGISTERED && s) {
				LM_DBG("No existing impu record for <%.*s>.... creating new "
					   "one\n",
						impu_str->len, impu_str->s);
				res = insert_impurecord(_d, impu_str, &private_identity,
						reg_state, barring, &subs_ptr, ccf1, ccf2, ecf1, ecf2,
						_r);
				if(res != 0) {
					...
					(*_r)->is_primary =
							is_primary; //TODO = this should prob move to insert_impurecord fn
				...
				LM_DBG("no IMPU found to update and data not valid to create "
					   "new one - not a problem record was probably removed as "
					   "it has no contacts\n");
				if(subscription_locked) {
					unref_subscription_unsafe(subs_ptr);
					unlock_subscription(subs_ptr);
	...
	if(send_sar_on_delete >= 0)
		(*_r)->send_sar_on_delete = send_sar_on_delete;
	...
	if(subs_ptr) {
		...
		if((*_r)->s != subs_ptr) {
			...
			LM_DBG("new subscription is the same as the old one....not doing "
				   "anything\n");
	...
	if(db_mode == WRITE_THROUGH
			&& db_insert_impurecord(_d, &(*_r)->public_identity,
					   (*_r)->reg_state, (*_r)->barring, &(*_r)->s,
					   &(*_r)->ccf1, &(*_r)->ccf2, &(*_r)->ecf1, &(*_r)->ecf2,
					   _r)
					   != 0) {
...
int remove_impucontact_from_list(
		impurecord_t *impu, impu_contact_t *impucontact)
{
 ...
 */
int link_contact_to_impu(
		impurecord_t *impu, ucontact_t *contact, int write_to_db)
{
	...
	while(impu_contact_ptr) {
		if(impu_contact_ptr->contact == contact) {
			LM_DBG("contact [%p] => [%.*s] already linked to impu [%.*s]\n",
					contact, contact->c.len, contact->c.s,
	...
	if(contact->is_3gpp && (maxcontact_behaviour > 0) && (maxcontact_3gpp > 0)
			&& (maxcontact_3gpp
					< (impu->linked_contacts.num3gppcontacts + 1))) {
		...
		if(impu_contact_ptr) {
			if(impu_contact_ptr == impu->linked_contacts.head) {
				impu->linked_contacts.head = impu->linked_contacts.tail =
						impu->linked_contacts.head->next;
			...
			if(write_to_db && db_mode == WRITE_THROUGH
					&& db_unlink_contact_from_impu(
							   impu, impu_contact_ptr->contact)
							   != 0) {
				LM_ERR("Failed to un-link DB contact [%.*s] from IMPU "
					   "[%.*s]...continuing but db will be out of sync!\n",
						impu_contact_ptr->contact->c.len,
						impu_contact_ptr->contact->c.s,
						impu->public_identity.len, impu->public_identity.s);
			}
			...
			if(locked == 0) {
				...
				LM_ERR("Could not get lock to remove link from of contact from "
					   "impu...\n");
	...
	if(!space_made && (maxcontact_behaviour > 0) && (maxcontact > 0)
			&& (maxcontact < (impu->linked_contacts.numcontacts + 1))) {
		...
		if(write_to_db && db_mode == WRITE_THROUGH
				&& db_unlink_contact_from_impu(impu, impu_contact_ptr->contact)
						   != 0) {
			LM_ERR("Failed to un-link DB contact [%.*s] from IMPU "
				   "[%.*s]...continuing but db will be out of sync!\n",
					impu_contact_ptr->contact->c.len,
					impu_contact_ptr->contact->c.s, impu->public_identity.len,
	...
	impu_contact_ptr =
			(impu_contact_t *)shm_malloc(sizeof(struct impu_contact));
	...
	LM_DBG("number of contacts for IMPU [%.*s] is %d\n",
			...
			impu->linked_contacts.numcontacts);
	if(write_to_db && db_mode == WRITE_THROUGH
			&& db_link_contact_to_impu(impu, contact) != 0) {
		LM_ERR("Failed to update DB linking contact [%.*s] to IMPU "
			   "[%.*s]...continuing but db will be out of sync!\n",
...
int unlink_contact_from_impu(impurecord_t *impu, ucontact_t *contact,
		int write_to_db, int is_explicit)
{
...
void ref_subscription_unsafe(ims_subscription *s)
{
	LM_DBG("Reffing subscription [%.*s] - was [%d]\n", s->private_identity.len,
			s->private_identity.s, s->ref_count);
	s->ref_count++;
}
...
void unref_subscription_unsafe(ims_subscription *s)
{
	...
	if(s->ref_count == 0) {
		if(s->sl
				>= 0) { //-1 as sl means the subscription was never added to the list
			sl = s->sl;
