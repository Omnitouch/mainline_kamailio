		...
		int is_timeout, void *param, AAAMessage *maa, long elapsed_msecs)
{
	...
	auth_data = 0;
	int item_number;
	str authenticate = {0, 0}, authorization2 = {0, 0}, ck = {0, 0},
		ik = {0, 0}, ip = {0, 0}, ha1 = {0, 0};
	...
	avp = cxdx_get_next_public_identity(
			maa, 0, AVP_IMS_Public_Identity, IMS_vendor_id_3GPP, __FUNCTION__);
	if(avp) {
	...
	if(sip_number_auth_items > 0) {
		...
		while((cxdx_get_auth_data_item_answer(maa, &auth_data, &item_number,
				&algorithm, &authenticate, &authorization2, &ck, &ik, &ip, &ha1,
				&response_auth, &digest_realm, &line_identifier))) {

			//create an auth_data_item for each entry in the MAA
			adi_len = sizeof(struct auth_data_item) + authenticate.len
					  + authorization2.len + ck.len + ik.len + ip.len + ha1.len
					  + line_identifier.len + response_auth.len
					  + digest_realm.len + algorithm.len;
		...
		case -1:
			switch(experimental_rc) {
				case RC_IMS_DIAMETER_ERROR_USER_UNKNOWN:
					stateful_request_reply_async(
							t, req, 403, MSG_403_USER_UNKNOWN);
					result = CSCF_RETURN_FALSE;
					break;
				case RC_IMS_DIAMETER_ERROR_IDENTITIES_DONT_MATCH:
					stateful_request_reply_async(
							t, req, 403, MSG_403_IDENTITIES_DONT_MATCH);
					result = CSCF_RETURN_FALSE;
					break;
				case RC_IMS_DIAMETER_ERROR_AUTH_SCHEME_NOT_SUPPORTED:
					stateful_request_reply_async(
							t, req, 403, MSG_403_AUTH_SCHEME_UNSOPPORTED);
					result = CSCF_RETURN_FALSE;
					break;

				default:
					stateful_request_reply_async(
							t, req, 403, MSG_403_UNKOWN_EXPERIMENTAL_RC);
					result = CSCF_RETURN_FALSE;
			}
			break;
	...
	if(avlist) {
		if(!data->is_resync) //only start the timer if we used the vector above - we don't use it resync mode
			start_reg_await_timer(
					avlist[0]); //start the timer to remove stale or unused Auth Vectors
 ...
 */
int cxdx_send_mar(struct sip_msg *msg, str public_identity,
		str private_identity, unsigned int count, str algorithm,
		...
		saved_transaction_t *transaction_data)
{
	...
	if(cxdx_dest_host.len > 0) {
		if(!cxdx_add_destination_host(mar, cxdx_dest_host))
			goto error1;
	}

	if(!cxdx_add_destination_realm(mar, cxdx_dest_realm))
		goto error1;

	if(!cxdx_add_vendor_specific_appid(
			   mar, IMS_vendor_id_3GPP, IMS_Cx, 0 /*IMS_Cx*/))
		goto error1;
	if(!cxdx_add_auth_session_state(mar, 1))
		goto error1;

	if(!cxdx_add_public_identity(mar, public_identity))
		goto error1;
	if(!cxdx_add_user_name(mar, private_identity))
		goto error1;
	if(!cxdx_add_sip_number_auth_items(mar, count))
		goto error1;
	if(algorithm.len == auth_scheme_types[AUTH_HTTP_DIGEST_MD5].len
			&& strncasecmp(algorithm.s,
					   auth_scheme_types[AUTH_HTTP_DIGEST_MD5].s, algorithm.len)
					   == 0) {
		if(!cxdx_add_sip_auth_data_item_request(mar, algorithm, authorization,
				   private_identity, cxdx_dest_realm,
				   msg->first_line.u.request.method, server_name))
			goto error1;
	} else {
		if(!cxdx_add_sip_auth_data_item_request(mar, algorithm, authorization,
				   private_identity, cxdx_dest_realm,
				   msg->first_line.u.request.method, s_empty))
			goto error1;
	}
