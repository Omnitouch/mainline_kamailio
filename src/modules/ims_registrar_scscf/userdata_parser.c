...
static inline char ifc_tDefaultHandling2char(xmlChar *x)
{
	...
	if(errno == EINVAL) {
		while(x[0]) {
			if(x[0] == 'c' || x[0] == 'C')
				return 0; //SESSION_CONTINUED
			if(x[0] == 'r' || x[0] == 'R')
				return 1; //SESSION_TERMINATED
			x++;
		}
		return 0;
	} else
...
static inline char ifc_tDirectionOfRequest2char(xmlChar *x)
{
	...
	if(errno == EINVAL) {
		while(x[0]) {
			if(x[0] == 'o' || x[0] == 'O')
				return 0; //ORIGINATING_SESSION
			if(x[0] == 's' || x[0] == 'S')
				return 1; //TERMINATING_REGISTERED
			if(x[0] == 'u' || x[0] == 'U')
				return 2; //TERMINATING_UNREGISTERED
			x++;
		}
		return 0;
	} else
 ...
 * @returns 1 on success, 0 on failure
 */
static int parse_application_server(
		xmlDocPtr doc, xmlNodePtr node, ims_application_server *as)
{
	xmlNodePtr child;
	xmlChar *x;
	as->server_name.s = NULL;
	as->server_name.len = 0;
	as->default_handling = IFC_NO_DEFAULT_HANDLING;
	as->service_info.s = NULL;
	as->service_info.len = 0;
 ...
 * @returns 1 on success, 0 on failure
 */
static int parse_session_desc(
		xmlDocPtr doc, xmlNodePtr node, ims_session_desc *sd)
{
	xmlNodePtr child;
	xmlChar *x;
	sd->line.s = NULL;
 ...
 * @returns 1 on success, 0 on failure
 */
static int parse_spt(xmlDocPtr doc, xmlNodePtr node, ims_spt *spt_to,
		...
		if(child->type == XML_ELEMENT_NODE)
			switch(child->name[0]) {
				...
				case 'm': //method
					...
					break;
				case 'S':
				case 's': { //SIPHeader/SessionCase/SessionDescription
					switch(child->name[7]) {
						case 'E':
						case 'e': //SIP_HEADER
						...
						case 'c': //Session Case
							...
							break;
						case 'D':
						case 'd': //Session Description
							spt->type = IFC_SESSION_DESC;
							parse_session_desc(
									doc, child, &(spt->session_desc));
							saved = child;
							break;
					}
					...
					if(group != spt->group) {
						...
						switch(spt2->type) {
							case IFC_REQUEST_URI:
								...
								spt2->request_uri.s =
										shm_malloc(spt2->request_uri.len);
								if(!spt2->request_uri.s) {
									LM_ERR("Out of memory allocating %d "
										   "bytes\n",
											spt->request_uri.len);
									break;
								}
								memcpy(spt2->request_uri.s, spt->request_uri.s,
										spt->request_uri.len);
								break;
							case IFC_METHOD:
								...
								memcpy(spt2->method.s, spt->method.s,
										spt->method.len);
								break;
							case IFC_SIP_HEADER:
								parse_sip_header(
										doc, saved, &(spt2->sip_header));
								break;
 ...
 */
static int parse_trigger_point(
		xmlDocPtr doc, xmlNodePtr node, ims_trigger_point *tp)
{
	...
	for(child = node->children; child; child = child->next)
		if(child->type == XML_ELEMENT_NODE)
			switch(child->name[0]) {
				case 'C':
				case 'c': //ConditionTypeCNF
					x = xmlNodeListGetString(doc, child->xmlChildrenNode, 1);
					tp->condition_type_cnf = ifc_tBool2char(x);
					xmlFree(x);
					break;
				case 'S':
				case 's': //SPT - Service Point Trigger
 ...
 * @returns 1 on success, 0 on failure
 */
static int parse_filter_criteria(
		xmlDocPtr doc, xmlNodePtr node, ims_filter_criteria *fc)
{
	xmlNodePtr child;
	xmlChar *x;
	char k;
	...
	for(child = node->children; child; child = child->next)
		if(child->type == XML_ELEMENT_NODE)
			switch(child->name[3]) {
				...
				case 'G':
				case 'g': //TriggerPoint
					fc->trigger_point = (ims_trigger_point *)shm_malloc(
					...
					break;
				case 'L':
				case 'l': //ApplicationServer
					parse_application_server(
							doc, child, &(fc->application_server));
					break;
				case 'F':
				case 'f': //ProfilePartIndicator
					x = xmlNodeListGetString(doc, child->xmlChildrenNode, 1);
					k = ifc_tProfilePartIndicator2char(x);
					if(k < 0)
						break;
					fc->profile_part_indicator =
							(char *)shm_malloc(sizeof(char));
 ...
 */
static int parse_public_identity(
		xmlDocPtr doc, xmlNodePtr root, ims_public_identity *pi)
{
	xmlNodePtr child;
	xmlNodePtr grandson;
	xmlChar *x;
		...
		if(child->type == XML_ELEMENT_NODE)
			switch(child->name[0]) {
				...
				case 'e':
					...
					for(grandson = child->children; grandson;
							...
							switch(grandson->name[0]) {
								...
								case 'w':
									...
									if(!scscf_support_wildcardPSI) {
										LOG(L_ERR, "Configured without support "
												   "for Wildcard PSI and got "
												   "one from HSS\n");
										LOG(L_ERR,
												"the identity will be stored "
												"but never be matched, please "
												"include the parameter to "
												"support wildcard PSI in the "
												"config file\n");
									}

									x = xmlNodeListGetString(
											doc, grandson->xmlChildrenNode, 1);
									space_trim_dup(
											&(pi->wildcarded_psi), (char *)x);
 ...
 * @returns 1 on success, 0 on failure
 */
static int parse_cn_service_auth(
		xmlDocPtr doc, xmlNodePtr node, ims_cn_service_auth *cn)
{
	xmlNodePtr child;
	xmlChar *x;
 ...
 * @returns 1 on success, 0 on failure
 */
static int parse_service_profile(
		xmlDocPtr doc, xmlNodePtr root, ims_service_profile *sp)
{
	xmlNodePtr child;
	xmlChar *x;
		...
		if(child->type == XML_ELEMENT_NODE) {
			...
			switch(child->name[0]) {
				...
				case 'p':
					...
					break;
				case 'i':
				case 'I': //InitialFilterCriteria
					ifc_cnt++;
					break;
				case 'c':
				case 'C': //CoreNetworkServiceAuthorization
	...
	sp->filter_criteria = (ims_filter_criteria *)shm_malloc(
			sizeof(ims_filter_criteria) * ifc_cnt);
			...
			switch(child->name[0]) {
				...
				case 'i': //InitialFilterCriteria
					if(!parse_filter_criteria(doc, child, &(fctemp)))
						break;
					i = 0;
					while(i < sp->filter_criteria_cnt
							&& sp->filter_criteria[i].priority
									   < fctemp.priority)
						i++;
				...
				case 'C':
				case 'c': //CoreNetworkServiceAuthorization
					if(!parse_cn_service_auth(
							   doc, child, sp->cn_service_auth)) {
				...
				case 's': //SharedIFCSet
					x = xmlNodeListGetString(doc, child->xmlChildrenNode, 1);
					sp->shared_ifc_set[sp->shared_ifc_set_cnt++] =
							atoi((char *)x);
					xmlFree(x);
					break;
			}
...
static ims_subscription *parse_ims_subscription(xmlDocPtr doc, xmlNodePtr root)
{
	...
	if(!root)
		return 0;
	while(root->type != XML_ELEMENT_NODE
			|| strcasecmp((char *)root->name, "IMSSubscription") != 0) {
		root = root->next;
	}
	if(!root) {
		...
		if(child->type == XML_ELEMENT_NODE)
			switch(child->name[0]) {
				...
				case 'p': /* Private Identity */
					...
					break;
				case 'S':
				case 's': /* Service Profile */
					sp_cnt++;
					break;
			}
	s->service_profiles = (ims_service_profile *)shm_malloc(
	...
	for(child = root->children; child; child = child->next)
		if(child->type == XML_ELEMENT_NODE)
			if(child->name[0] == 'S' || child->name[0] == 's') {
				rc = parse_service_profile(doc, child,
						&(s->service_profiles[s->service_profiles_cnt]));
				if(rc == 2)
					s->wpsi = 1;
				if(rc)
...
ims_subscription *parse_user_data(str xml)
{
	...
	if(!ctxtInit)
		parser_init(scscf_user_data_dtd, scscf_user_data_xsd);
	doc = 0;
...
int parser_init(char *dtd_filename, char *xsd_filename)
{
	...
	if(xsd_filename) {
		...
		xmlSchemaSetParserErrors(ctxt, (xmlValidityErrorFunc)fprintf,
				(xmlValidityWarningFunc)fprintf, stderr);
		xsd = xmlSchemaParse(ctxt);
		xmlSchemaFreeParserCtxt(ctxt);

		xsdCtxt = xmlSchemaNewValidCtxt(xsd);
		xmlSchemaSetValidErrors(xsdCtxt, (xmlValidityErrorFunc)fprintf,
				(xmlValidityWarningFunc)fprintf, stderr);
	}
	ctxtInit = 1;
	return 1;
