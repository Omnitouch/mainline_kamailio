...
#define GRACE_DISCON_TIMEOUT \
	60 /**< 60 seconds for a DISCON acct session to hang around for before being cleaned up */
...
void free_session(cdp_session_t *x)
{
	if(x) {
		...
		if(x->dest_host.s)
			shm_free(x->dest_host.s);
		if(x->dest_realm.s)
			shm_free(x->dest_realm.s);
...
void cdp_add_session(cdp_session_t *x)
{
	...
	if(sessions[x->hash].tail)
		sessions[x->hash].tail->next = x;
	sessions[x->hash].tail = x;
	if(!sessions[x->hash].head)
		sessions[x->hash].head = x;
}
...
cdp_session_t *cdp_get_session(str id)
{
	...
	for(x = sessions[hash].head; x; x = x->next) {
		LM_DBG("looking for |%.*s| in |%.*s|\n", id.len, id.s, x->id.len,
				x->id.s);
		if(x->id.len == id.len && strncasecmp(x->id.s, id.s, id.len) == 0)
			return x;
	}
...
void del_session(cdp_session_t *x)
{
	...
	if(sessions[x->hash].head == x)
		sessions[x->hash].head = x->next;
	else if(x->prev)
		x->prev->next = x->next;
	if(sessions[x->hash].tail == x)
		sessions[x->hash].tail = x->prev;
	else if(x->next)
		x->next->prev = x->prev;
...
static int generate_session_id(str *id, unsigned int end_pad_len)
{
	...
	id->len = config->identity.len
			  + 1 /*;*/ + 10 /*high 32 bits*/ + 1 /*;*/ + 10 /*low 32 bits*/ +
			  //		1/*;*/ + 8/*optional value*/ +
			  1 /* terminating \0 */ + end_pad_len;
	...
	*session_id2 = s2;
	...
	/* build the sessionID */
	sprintf(id->s, "%.*s;%u;%u", config->identity.len, config->identity.s,
			*session_id1, s2);
	id->len = strlen(id->s);
	return 1;
error:
		...
		for(hash = 0; hash < sessions_hash_size; hash++) {
			...
			for(x = sessions[hash].head; x; x = x->next) {
				LM_DBG(" %3u. [%.*s] AppId [%d] Type [%d]\n", hash, x->id.len,
						x->id.s, x->application_id, x->type);
				switch(x->type) {
					case AUTH_CLIENT_STATEFULL:
					case AUTH_SERVER_STATEFULL:
						LM_DBG("Auth State [%d] Timeout [%d] Lifetime [%d] "
							   "Grace [%d] Generic [%p] Class [%d]\n",
								x->u.auth.state,
			...
			switch(x->type) {
				case ACCT_CC_CLIENT:
					if(x->u.cc_acc.type == ACC_CC_TYPE_SESSION) {
						...
						if((time(0) > (x->u.cc_acc.discon_time
									   + GRACE_DISCON_TIMEOUT))
								&& (x->u.cc_acc.state == ACC_CC_ST_DISCON)) {
							...
							if(last_res_timestamp && res_valid_for) {
								...
								if((last_res_timestamp + res_valid_for)
										< (time(0) + last_reservation
												+ buffer_time)) {
									LM_DBG("reservation about to expire, "
										   "sending callback\n");
									cc_acc_client_stateful_sm_process(
											x, ACC_CC_EV_RSVN_WARNING, 0);
								}
							}
						}
				...
				case AUTH_CLIENT_STATEFULL:
					if(x->u.auth.timeout >= 0 && x->u.auth.timeout <= now) {
						...
						auth_client_statefull_sm_process(
								x, AUTH_EV_SESSION_TIMEOUT, 0);
					} else if(x->u.auth.lifetime > 0
							  && x->u.auth.lifetime + x->u.auth.grace_period
										 <= now) {
						//lifetime + grace timeout
						LM_CRIT("lifetime+grace TIMEOUT\n");
						auth_client_statefull_sm_process(
								x, AUTH_EV_SESSION_GRACE_TIMEOUT, 0);
					} else if(x->u.auth.lifetime > 0
							  && x->u.auth.lifetime <= now) {
						//lifetime timeout
						LM_CRIT("lifetime+grace TIMEOUT\n");
						auth_client_statefull_sm_process(
								x, AUTH_EV_SESSION_LIFETIME_TIMEOUT, 0);
					}
					break;
				case AUTH_SERVER_STATEFULL:
					if(x->u.auth.timeout >= 0 && x->u.auth.timeout <= now) {
...
AAASession *cdp_new_auth_session(str id, int is_client, int is_statefull)
{
	...
	if(is_client) {
		if(is_statefull)
			type = AUTH_CLIENT_STATEFULL;
		else
			type = AUTH_CLIENT_STATELESS;
	} else {
		if(is_statefull)
			type = AUTH_SERVER_STATEFULL;
		else
			type = AUTH_SERVER_STATELESS;
	}
	s = cdp_new_session(id, type);
	if(s) {
		...
		s->u.auth.last_requested_grace = s->u.auth.last_requested_lifetime =
				s->u.auth.last_requested_timeout = 0;
		cdp_add_session(s);
	}
	return s;
}
 ...
 * @returns the new AAASession or null on error
 */
AAASession *AAACreateClientAuthSession(
		int is_statefull, AAASessionCallback_f *cb, void *generic_data)
{
	AAASession *s;
	str id;
 ...
 * @returns the new AAASession or null on error
 */
AAASession *AAACreateServerAuthSession(AAAMessage *msg, int is_statefull,
		AAASessionCallback_f *cb, void *generic_data)
{
	AAASession *s;
	str id;
 ...
 * @returns the new AAASession or null on error
 */
AAASession *AAACreateCCAccSession(
		AAASessionCallback_f *cb, int is_session, void *generic_data)
{
	AAASession *s;
	str id;
...
int AAAStartChargingCCAccSession(AAASession *s)
{
	if(s->type != ACCT_CC_CLIENT && s->u.cc_acc.type != ACC_CC_TYPE_SESSION) {
		LM_ERR("Can't start charging on a credit-control session that is not "
			   "session based\n");
		return -1;
	}
